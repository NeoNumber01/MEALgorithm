export const CACHE_KEYS = {
    ...
    DASHBOARD_FEEDBACK_TIME: 'dashboard_feedback_time', // Time when feedback was generated
...
// Invalidate AI feedback cache to force regeneration
export function invalidateAIFeedbackCache() {
    if (typeof window === 'undefined') return
    localStorage.removeItem(CACHE_KEYS.DASHBOARD_FEEDBACK)
    localStorage.removeItem(CACHE_KEYS.DASHBOARD_FEEDBACK_HASH)
    localStorage.removeItem(CACHE_KEYS.DASHBOARD_FEEDBACK_TIME)

    // Also clear stats insight caches (all keys starting with prefix)
    const keysToRemove: string[] = []
    for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i)
        if (key && (key.startsWith('stats_insight_') || key.startsWith('stats_data_'))) {
...
export function shouldRegenerateAIFeedback(
    ...
    // Check if data was updated after last feedback
    const lastDbUpdate = getLastDataUpdateTime()
    if (lastDbUpdate > lastFeedbackTime) return true
